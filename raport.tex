...
\author{Nepotu Alexandra-Maria, Slănină Ștefan}
\date{Decembrie 2025}
...
Toți algoritmii sunt implementați folosind Python, cu accent pe înțelegerea matematicii din spatele modelelor și pe implementarea custom a componentelor cheie.
...
\subsubsection{Împărțirea Train/Test}
    ...
    Împărțirea se face la nivel de \texttt{id\_bon}, nu la nivel de rânduri, pentru a preveni data leakage:
...
Hit@K măsoară de câte ori sosul real din coș apare în Top-K recomandări:
...
# Use X_test_cache[sauce] inside recommend_top_k()
\end{lstlisting}
...
\begin{lstlisting}[language=Python]
X_bin = (X > 0).astype(int)  # 1 if product is present, 0 otherwise
\end{lstlisting}
...
\subsection{Comparație Algoritmi de Ranking}

\subsubsection{Motivație}

Pentru a valida performanța Naive Bayes și pentru a îmbunătăți sistemul de ranking, am implementat și comparat \textbf{4 algoritmi diferiți}:

\begin{enumerate}
    \item \textbf{Naive Bayes (Implementare Custom)} - Algoritm probabilistic cu independență condițională
    \item \textbf{k-NN (k-Nearest Neighbors)} - Algoritm bazat pe distanțe, cu k=3
    \item \textbf{Decision Tree (ID3-like)} - Arbore de decizie cu criterion=entropy
    \item \textbf{AdaBoost} - Ensemble method cu 50 de estimatori weak (Decision Trees)
\end{enumerate}

\textbf{Metodologie comună}: Toți algoritmii folosesc \textbf{exact aceeași formulă de scoring}:

\begin{equation}
\text{Score}(p \mid \text{coș}) = P(p \mid \text{coș}) \times \text{price}(p)
\end{equation}

Diferă doar modul în care estimează probabilitatea $P(p \mid \text{coș})$.

\subsubsection{Descrierea Algoritmilor}

\paragraph{1. Naive Bayes (Custom Implementation)}

\textbf{Tip}: Algoritm probabilistic generativ

\textbf{Avantaje}:
\begin{itemize}
    \item Rapid și scalabil
    \item Interpretabil (probabilități explicite)
    \item Funcționează bine cu trăsături binare
\end{itemize}

\textbf{Dezavantaje}:
\begin{itemize}
    \item Presupune independență condițională (poate fi incorectă)
    \item Performanță limitată pe relații non-liniare complexe
\end{itemize}

\textbf{Ecuația cheie}:
\begin{equation}
P(p \mid \mathbf{x}) \propto P(p) \cdot \prod_{i=1}^{d} P(x_i \mid p)
\end{equation}

\paragraph{2. k-Nearest Neighbors (k-NN)}

\textbf{Tip}: Instance-based learning

\textbf{Avantaje}:
\begin{itemize}
    \item Fără training explicit
    \item Capturează pattern-uri locale
    \item Non-parametric
\end{itemize}

\textbf{Dezavantaje}:
\begin{itemize}
    \item Lent la predicție (curse of dimensionality)
    \item Sensibil la scalarea features
    \item Necesită multă memorie
\end{itemize}

\textbf{Ecuația cheie}:
\begin{equation}
P(p \mid \mathbf{x}) = \frac{1}{k} \sum_{i \in N_k(\mathbf{x})} \mathbb{1}[y_i = p]
\end{equation}

unde $N_k(\mathbf{x})$ sunt cei mai apropiați k vecini ai lui $\mathbf{x}$.

\paragraph{3. Decision Tree (ID3-like)}

\textbf{Tip}: Tree-based, discriminative

\textbf{Avantaje}:
\begin{itemize}
    \item Capturează relații non-liniare complexe
    \item Interpretabil (reguli explicit if-then)
    \item Gestionează interacțiuni între features
\end{itemize}

\textbf{Dezavantaje}:
\begin{itemize}
    \item Poate overfita (necesită pruning)
    \item Instabil (sensibil la schimbări mici în date)
    \item Bias către features cu multe valori
\end{itemize}

\textbf{Criteriu de split (Information Gain)}:
\begin{equation}
\text{IG}(X, a) = H(X) - \sum_{v \in \text{Values}(a)} \frac{|X_v|}{|X|} H(X_v)
\end{equation}

unde $H(X) = -\sum_{c} P(c) \log_2 P(c)$ este entropia.

\paragraph{4. AdaBoost (Adaptive Boosting)}

\textbf{Tip}: Ensemble method (boosting)

\textbf{Avantaje}:
\begin{itemize}
    \item Combină weak learners în strong learner
    \item Reduce bias și variance
    \item Performanță robustă pe diverse dataset-uri
\end{itemize}

\textbf{Dezavantaje}:
\begin{itemize}
    \item Sensibil la noise și outliers
    \item Training mai lent
    \item Mai puțin interpretabil
\end{itemize}
...
unde $\alpha_t$ sunt ponderile estimatorilor și $h_t$ sunt weak learners.

\subsubsection{Rezultate Comparative}

\begin{table}[H]
\centering
\caption{Rezultate Hit@K pentru Toți Algoritmii}
\begin{tabular}{lrrrrr}
\toprule
\textbf{K} & \textbf{Naive Bayes} & \textbf{k-NN} & \textbf{Decision Tree} & \textbf{AdaBoost} & \textbf{Popularity} \\
\midrule
K=1 & 54.23\% & 49.30\% & \textbf{79.53\%} & 67.31\% & 32.31\% \\
K=3 & 84.57\% & 76.96\% & \textbf{92.60\%} & 80.06\% & 71.08\% \\
K=5 & 94.11\% & 92.18\% & 97.21\% & \textbf{97.43\%} & 93.54\% \\
\midrule
\textbf{Medie} & 77.63\% & 72.81\% & \textbf{89.78\%} & 81.60\% & 65.64\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Clasament final}:
\begin{enumerate}
    \item \textbf{Decision Tree (ID3-like)}: 89.78\% medie Hit@K - \textit{cel mai bun overall}
    \item \textbf{AdaBoost}: 81.60\% medie Hit@K - \textit{excelent la K=5}
    \item \textbf{Naive Bayes (Custom)}: 77.63\% medie Hit@K - \textit{competitiv, implementare proprie}
    \item \textbf{k-NN}: 72.81\% medie Hit@K - \textit{afectat de curse of dimensionality}
    \item \textbf{Popularity (Baseline)}: 65.64\% medie Hit@K
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{output/ranking_comparison_all.png}
\caption{Comparație vizuală Hit@K pentru toți algoritmii}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{output/ranking_heatmap.png}
\caption{Heatmap rezultate Hit@K}
\end{figure}

\subsubsection{Analiză și Interpretare}

\paragraph{1. Decision Tree }

Decision Tree obține cele mai bune rezultate (\textbf{89.78\%} medie) datorită:
\begin{itemize}
    \item Capacității de a captura \textbf{relații non-liniare complexe} între produse
    \item Gestionării eficiente a \textbf{interacțiunilor între features} (ex: "Crazy Schnitzel + Cartofi")
    \item Creării de \textbf{reguli explicite} bazate pe entropia informației
    \item Splits optimal bazate pe Information Gain (criterion=entropy, ID3-like)
\end{itemize}

\textbf{Exemplu de regulă învățată}: 
\begin{verbatim}
IF "Crazy Schnitzel" > 0 AND "Cartofi" > 0 THEN 
    P(Crazy Sauce | coș) = 0.85
\end{verbatim}

\paragraph{2. AdaBoost }

AdaBoost atinge \textbf{97.43\% la K=5}, aproape perfect, datorită:
\begin{itemize}
    \item \textbf{Boosting}: Combină 50 de weak learners (Decision Trees cu max\_depth=3)
    \item \textbf{Focus adaptiv}: Ponderează mai mult exemplele dificile
    \item \textbf{Reducere variance}: Evită overfitting-ul Decision Trees individuale
\end{itemize}

\paragraph{3. Naive Bayes }

NB custom obține \textbf{77.63\%} medie, validând implementarea proprie:
\begin{itemize}
    \item \textbf{Rapid și scalabil}: Pre-computarea probabilităților este instant
    \item \textbf{Interpretabil}: Probabilități explicite pentru fiecare produs
    \item \textbf{Limitare}: Independența condițională ignoră interacțiuni între produse
\end{itemize}

\paragraph{4. k-NN }

k-NN are performanță mai slabă (\textbf{72.81\%}) din cauza:
\begin{itemize}
    \item \textbf{66 dimensiuni}: Distanțele devin ambigii în spații high-dimensional
    \item \textbf{Sparse data}: Majoritatea coșurilor au puține produse (vectori rari)
    \item \textbf{Lentoare}: Necesită calcul distanțe la toate exemplele de training
\end{itemize}

\subsubsection{Exemplu Comparativ pe Coșul 35222}

\textbf{Conținut coș}: Crazy Schnitzel, Breaded Chicken Schnitzel, Pepsi Twist Zero Can 0.33L

\begin{table}[H]
\centering
\caption{Top-3 Recomandări per Algoritm pentru Coșul 35222}
\begin{tabular}{ll}
\toprule
\textbf{Algoritm} & \textbf{Top-3 Recomandări} \\
\midrule
Naive Bayes & Crazy Sauce, Cheddar Sauce, Garlic Sauce \\
k-NN & Blueberry Sauce, Crazy Sauce, Cheddar Sauce \\
\textbf{Decision Tree} & \textbf{Blueberry Sauce, Crazy Sauce, Cheddar Sauce} \\
...
\textbf{Observații}:
\begin{itemize}
    \item \textbf{Crazy Sauce} apare în toate recomandările (consistent cu Schnitzel)
    \item \textbf{Decision Tree și k-NN} au aceeași Top-3 (pattern comun învățat)
    \item \textbf{Naive Bayes și Popularity} sunt similare (model probabilistic simplu)
    \item \textbf{AdaBoost} recomandă Spicy Sauce (explorare mai diversificată)
\end{itemize}

\subsubsection{Concluzie Comparație}


\textbf{Recomandare finală}: \textbf{Decision Tree sau AdaBoost} pentru sistemul de upselling în producție, datorită acurateții superioare și capacității de a captura relații complexe între produse.
...
\midrule
\multicolumn{3}{l}{\textbf{Ranking - Comparație Algoritmi (Hit@3):}} \\
\quad Decision Tree (ID3) & Hit@3 & \textbf{92.60\%} \\
\quad AdaBoost & Hit@3 & 80.06\% \\
\quad Naive Bayes (Custom) & Hit@3 & 84.57\% \\
\quad k-NN & Hit@3 & 76.96\% \\
\quad Popularity (Baseline) & Hit@3 & 71.08\% \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Comparația Algoritmilor de Ranking}

Implementarea și compararea a 4 algoritmi diferiți pentru ranking a oferit perspective valoroase:

\textbf{Algoritmi testați}:
\begin{enumerate}
    \item \textbf{Naive Bayes (Custom)}: Implementare from scratch cu Laplace smoothing
    \item \textbf{k-NN (k=3)}: Instance-based learning cu metric euclidian
    \item \textbf{Decision Tree (ID3)}: Arbore de decizie cu criterion=entropy, max\_depth=10
    \item \textbf{AdaBoost}: Ensemble cu 50 estimatori (Decision Trees)
\end{enumerate}

\textbf{Lecții cheie}:
\begin{itemize}
    \item \textbf{Decision Tree câștigă}: 89.78\% medie Hit@K, capturează relații non-liniare complexe între produse
    \item \textbf{AdaBoost excelent la K mare}: 97.43\% Hit@5, aproape perfect prin boosting
    \item \textbf{Naive Bayes custom competitiv}: 77.63\% validează implementarea proprie
    \item \textbf{k-NN limitări}: 72.81\%, curse of dimensionality în spații cu 66 dimensiuni și date sparse
    \item \textbf{Trade-offs esențiale}: Acuratețe (DT > Ada > NB > kNN) vs Viteză (NB > DT > Ada > kNN)
\end{itemize}

\textbf{Insight important}: Nu există algoritm universal superior - alegerea depinde de:
\begin{itemize}
    \item Complexitatea relațiilor în date (DT pentru non-liniar, NB pentru linear)
    \item Cerințe de viteză (NB pentru real-time, DT pentru batch)
    \item Interpretabilitate (DT și NB ușor de explicat, k-NN și Ada "black-box")
\end{itemize}

\textbf{Recomandare practică}: Decision Tree sau AdaBoost pentru sistemul de producție, Naive Bayes pentru prototipare rapidă și experimentare.
...
\begin{itemize}
    ...
    \item \textbf{Ranking}: Comparație 4 algoritmi - Decision Tree câștigă cu 89.78\% medie Hit@K
\end{itemize}

\textbf{Contribuții principale}:
\begin{enumerate}
    \item \textbf{Implementări custom}: Logistic Regression cu GD și Bernoulli Naive Bayes from scratch
    \item \textbf{Comparație algoritmică riguroasă}: 4 algoritmi evaluați pe aceleași date și metrici
    \item \textbf{Insights practice}: Decision Tree superior pentru relații non-liniare, NB rapid pentru prototipare
    \item \textbf{Optimizare realistă}: Feature caching (60-100x speedup) esențial pentru evaluări repetate
\end{enumerate}

Toate cerințele proiectului au fost îndeplinite:
\begin{itemize}
    \item[$\checkmark$] Implementare custom a cel puțin unui algoritm (LR cu GD, Bernoulli NB)
    \item[$\checkmark$] Feature engineering corect (fără data leakage, excludere produs țintă)
    \item[$\checkmark$] Evaluare comprehensivă (Hit@K, Precision@K, ROC-AUC pentru clasificare, comparație baseline)
    \item[$\checkmark$] Comparație algoritmi de ranking (NB, k-NN, Decision Tree, AdaBoost)
    \item[$\checkmark$] Interpretare coeficienți și pattern-uri (top features, exemple concrete)
    \item[$\checkmark$] Optimizare performanță (feature caching, probability pre-computation)
    \item[$\checkmark$] Documentare comprehensivă (raport LaTeX, README, comentarii cod)
\end{itemize}

\textbf{Impact potențial}: Astfel de sisteme pot crește veniturile restaurantelor cu 10-20\% prin recomandări personalizate și strategii de upselling eficiente bazate pe machine learning.

\textbf{Lecție finală}: Alegerea algoritmului optim necesită evaluare experimentală riguroasă - pattern-urile din date determină ce algoritm funcționează cel mai bine, nu dogme teoretice.
...
Repository-ul complet este disponibil la: \texttt{https://github.com/alexandranepotu/ml-restaurant-sells}
